...
let check (globals, functions) =
  ...
  let check_binds (kind : string) (binds : bind list) =
    List.iter (function
	Dec(Void, b) | Decinit(Void, b, _) -> raise (Failure ("illegal void " ^ kind ^ " " ^ b))
      | _ -> ()) binds;
    let rec dups = function
        [] -> ()
      |	(b1 :: b2 :: _) when name_of_bind b1 = name_of_bind b2 ->
	  raise (Failure ("duplicate " ^ kind ^ " " ^ name_of_bind b1))
      | _ :: t -> dups t
    in dups (List.sort (fun a b -> compare (name_of_bind a) (name_of_bind b)) binds)
  ...
  let check_function func =
    ...
    let check_formals (formals : formal list) =
    List.iter (function
	(Void, b) -> raise (Failure ("illegal void formal " ^ b))
      | _ -> ()) formals;
    let rec dups = function
        [] -> ()
